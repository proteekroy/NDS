...
public class DivideAndConquer extends AbstractNDS {
	
	FasterNonDominatedSorting  fans;
	
	public DivideAndConquer (int arg_n, int arg_m)
	{
		n = arg_n;
		m = arg_m;
		rank = new int[n];
		...
		fans = new FasterNonDominatedSorting();
	}
	...
	public void sort(double [][] population) 
	{
		...
		endTime2 = start;
		FasterNonDominatedSorting.Sorter x = fans.getSorter(n, m);
		x.sortImpl(population, rank);
		end = System.nanoTime();
		mini_stat();
		if(printinfo)
		{
	...
	public void initialize(double[][] pop)
	{
		if(printinfo)
		{
			System.out.println("\nStarting DivideAndConquer Sort");
		}
		population = pop;
		totalfront = 0;
		comparison = 0;
	...
	private class FasterNonDominatedSorting {
	    /**
	    ...
	    private final class SorterEmpty extends Sorter {
	    ...
	    private  final class Sorter0D extends Sorter {
	    ...
	    private class Sorter1D extends Sorter {
	    ...
	    private class Sorter2D extends Sorter {
	        private final int[] indices;
	    ...
	    private class SorterXD extends Sorter {
	        private final int[] indices;
